...
## LETS TURN IT INTO F&W Boundaries instead!!!!!!
```{r}
# Ok, now i need to combine them by F&W boundaries, instead of PMEP Boundaries!

unique(north_sub_mpa$study_regi)
```
```{r}
unique(central_sub_mpa$study_regi)
```

```{r}
unique(south_sub_mpa$study_regi)
```
...
```{r}
unique(ncsr_substrate$study_regi)
```
...
depths_with_sub <- substrate |> 
  select("pmep_zone_detail", "Shape", "pmep_region", "area_ha", "shape_area", "cmecs_sc_name", "cmecs_sc_category") |> 
  mutate(pmep_zone_detail = case_when(
    pmep_zone_detail == "Deep Shelf or Sound (-100m to -200m), State Waters"  ~ "Zone 6",
    pmep_zone_detail == "Outside PMEP Scope (>-200m) or International Waters" ~ "Zone 8",
    pmep_zone_detail == "Seaward Zone (-30m to -100m), Federal"               ~ "Zone 5",
...
**Note about filtering below - I don't see an area_ha column, like there was in the substrate RDS. Check with Maddy about adding this to the rds?**
```{r}
# To be assigned units, it must be projected!
biota <- st_transform(biota, crs = 32610) # this will put the shape_area into m2?
```


```{r}
# Make a column in the biota dataframe for area in hectares 
biota <- biota |> 
...
```{r}
# Filter biota to just the categories we need for this viz
depths_with_biota <- biota |> 
  select("pmep_zone", "Shape", "pmep_region", "shape_area", "area_ha", "cmecs_bc_name", "cmecs_bc_category")

depths_with_biota_simple <- biota |> 
  select("pmep_zone", "Shape", "pmep_region", "shape_area", "area_ha", "cmecs_bc_name", "cmecs_bc_category") |> 
  st_drop_geometry()

unique(depths_with_biota$pmep_zone)
```
```{r}
...
**Seems like theres something wrong here...with what numbers are in the different area columns. Need to troubleshoot...**
