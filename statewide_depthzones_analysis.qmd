...
```{r}
# Join mpas with sr_boundaries datasets, so that there is no sr_boundaries data where the mpa data is, so that when you click on an mpa in the interactive map, it shows the mpa name instead of the region name. 
```
