...
```{r}
# Lets try and add a biota layer to zone 0, to complete this thought experiment
colnames(biota)
```
```{r}
...
```{r}
unique(biota$cmecs_bc_name)
```

```{r}
unique(substrate$cmecs_sc_category)
```
**Note about filtering below - I don't see an area_ha column, like there was in the substrate RDS. Check with Maddy?**

```{r}
# Filter biota to just the categories we need for this viz
depths_with_biota <- biota |> 
  select("pmep_zone", "Shape", "pmep_region", "shape_area", "cmecs_bc_name", "cmecs_bc_category")
